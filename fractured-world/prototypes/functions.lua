...
local function slider_to_scale(autoplaceControlName, range)
    range = range or 6
    return (1 - noise.log2(noise.var(autoplaceControlName)) /
...
local function dot(vec1, vec2)
    ...
    if #vec1 ~= #vec2 then
        error("Vectors of different dimesion cannot be multiplied")
    end
...
local function get_extremum(func, values)
    if func == "max" then
        return reduce(function(a, b) return noise.clamp(a, b, math.huge) end,
                      values)
    elseif func == "min" then
        return reduce(function(a, b) return noise.clamp(a, -math.huge, b) end,
                      values)
    end
end
...
local function rotate_map(x, y)
    ...
    local largeRotationFactor = slider_to_scale(
...
local defaultSize =
    settings.startup["fractured-world-default-cell-size"].value *
