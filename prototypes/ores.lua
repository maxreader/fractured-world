...
local starting_factor = noise.delimit_procedure(
                            noise.clamp(
                                -noise.min(radius, 0) *
                                    math.huge, 0, 1))
local startingPatchScaleFactor =
    (startingAreaRadius / 128) ^ 0.5, 1
local startingPatchDefaultRadius =
    15 * startingPatchScaleFactor
...
local starting_patches =
    resource_autoplace__patch_metasets.starting
        .patch_set_indexes
local regular_patches =
...
for name, ore in pairs(resources) do
    local needsDefault = (ore.autoplace and ore.autoplace ~=
                             {} and
                             not currentResourceData[name]) and
                             true or false
...
local doInfiniteOres =
    settings.startup["fractured-world-enable-infinite-parenting"]
        .value
local startingOreCount = 0
for ore, oreData in pairs(currentResourceData) do
    local isInfinite = doInfiniteOres and
                           resources[ore].infinite and
                           string.find(ore, "^infinite%-") and
                           true or false
    if isInfinite then
        ...
        if resources[parentOreName] then
            ...
            currentResourceData[parentOreName]
                .has_infinite_version = true
        end
    end
    local is_starting_patch = (starting_patches[ore] or
                                  resources[ore].autoplace
                                      .starting_area == true or
                                  currentResourceData[ore]
                                      .has_starting_area_placement ==
                                  true) and true or false
    if is_starting_patch then
        startingOreCount = startingOreCount + 1
        currentResourceData[ore].starting_patch =
            startingOreCount
    end
end
--[[
...
local smallRotationFactor = 1 - functions.slider_to_scale(
...
for ore, oreData in pairs(currentResourceData) do
    ...
    local frequency_multiplier =
        control_setting.frequency_multiplier or 1
    local base_spots_per_km2 =
        oreData.base_spots_per_km2 or 2.5
    local thisFrequency = frequency_multiplier *
                              base_spots_per_km2
    oreData.startLevel = oreCount
    oreCount = oreCount + thisFrequency
    oreData.endLevel = oreCount
    local random_spot_size_minimum =
        oreData.random_spot_size_minimum or 0.25
    local random_spot_size_maximum =
        oreData.random_spot_size_maximum or 2
    oreData.variance = random_spot_size_maximum -
                           random_spot_size_minimum
    oreData.random_spot_size_minimum =
        random_spot_size_minimum
    oreData.starting_rq_factor_multiplier_multiplier =
        1 +
            ((oreData.starting_rq_factor_multiplier or 1) -
                1) * tne(starting_factor)
end

local overallFrequency =
    settings.startup["fractured-world-overall-resource-frequency"]
        .value * noise.var(
        "control-setting:overall-resources:frequency:multiplier") ^
        2
local maxPatchesPerKm2 = overallFrequency * 64
local oreCountMultiplier = noise.delimit_procedure(
                               noise.max(1, oreCount /
                                             maxPatchesPerKm2))

-- scale startLevel and endLevel so that the desired overall frequency of islands have ore
for ore, oreData in pairs(currentResourceData) do
    oreData.startLevel = tne(oreData.startLevel) /
                             (oreCountMultiplier) *
                             overallFrequency
    oreData.endLevel = tne(oreData.endLevel) /
                           (oreCountMultiplier) *
                           overallFrequency
end
...
local function get_infinite_probability(ore)
    ...
    local parentProbability =
        data.raw["noise-expression"]["fractured-world-" ..
            parentOreName .. "-probability"].expression

    local minRadius = 1 / 8
    local maxRadius = 1 / 2
    local get_radius = functions.make_interpolation(
                           parentOreData.startLevel,
                           minRadius,
    ...
    data:extend{
        {
            ...
            name = "fractured-world-" .. ore ..
                "radial-multiplier",
            expression = noise.max(
                thisRadius - scaledRadius, 0)
        }
    }
    -- if the island is dry, *or* if it has biters on it, place the infinite ore
    local moistureFactor = noise.max(
                               noise.less_than(
                                   noise.var("moisture"),
                                   tne(0.5)), noise.var(
                                   "fractured-world-biter-islands"))
    local sizeMultiplier = noise.get_control_setting(ore)
                               .size_multiplier
    local randomness = noise.clamp(
                           noise.var("fw-scaling-noise"), 1,
                           10)
    local probabilities = {
        tne(10), parentProbability, moistureFactor,
        noise.var("fractured-world-" .. ore ..
                      "radial-multiplier"), sizeMultiplier,
        randomness
    }
...
local function get_infinite_richness(ore)
    local oreData = currentResourceData[infiniteOreData[ore]
                        .parentOreName]
    local additional_richness =
        oreData.additional_richness or 0
    local richness_post_multiplier =
        oreData.richness_post_multiplier or 1
    local minimumRichness = oreData.minRich or 0
    local settings = noise.get_control_setting(ore)
    local variance = (aux - oreData.startLevel) /
                         (oreData.endLevel -
                             oreData.startLevel) *
                         oreData.variance +
                         (oreData.random_spot_size_minimum)
    ...
    local factors = {
        ...
        noise.max(noise.var("fractured-world-" .. ore ..
                                "radial-multiplier"), 1),
        tne(10)
    }
    return noise.max((functions.multiply_probabilities(
                         factors) + additional_richness) *
                         richness_post_multiplier,
                     minimumRichness)
end
...
local function get_probability(ore)
    ...
    local settings = noise.get_control_setting(ore)
    local random_probability =
        oreData.random_probability or 1
    local aboveMinimum = noise.max(0,
                                   aux - oreData.startLevel)
    local belowMaximum =
        noise.max(0, oreData.endLevel - aux)
    local probability_expression =
        noise.clamp(aboveMinimum * belowMaximum * math.huge,
                    0, 1)
    probability_expression = probability_expression *
                                 (tne(1) - starting_factor)
    if oreData.starting_patch then
        local startingPatchRadius =
            startingPatchDefaultRadius *
                settings.size_multiplier ^ 0.5 *
                (oreData.starting_rq_factor_multiplier_multiplier or
                    1)
        local startingPoint =
            startingPoints[oreData.starting_patch]
        local point_x = startingPoint.x
        local point_y = startingPoint.y
        local x = noise.var("x")
        local y = noise.var("y")
        local distanceFromPoint =
            functions.distance(point_x - x, point_y - y)
        probability_expression =
            probability_expression +
                noise.less_than(distanceFromPoint,
                                startingPatchRadius +
                                    (noise.var(
                                        "fw-small-noise") /
                                        25))
...
local function get_richness(ore)
    ...
    local factors = {
        ...
        noise.clamp(noise.absolute_value(
                        noise.var("fw-small-noise") / 25 + 2),
                    0.5, 2),
